#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>

#define RTU 0x01
#define RHR 0x03
#define WHR 0x17
// static unsigned char auchCRCHi[] = {
static uint8_t auchCRCHi[] = {
    
};


// static char     auchCRCLo[] = {
static uint8_t auchCRCLo[] = {
    
};

uint16_t athCalcCRC(uint8_t n) {
    /* The function returns the CRC as a unsigned short type  */
    /* *puchMsg message to calculate CRC upon  */
    /* usDataLen  quantity of bytes in message   */

    /*
       uint8_t  *puchMsg;
       uint8_t  usDataLen;
       */
    static uint8_t  uchCRCHi = 0xFF;    /* high byte of CRC initialized   */
    static uint8_t   uchCRCLo = 0xFF;    /* low byte of CRC initialized   */

    static uint8_t uIndex = 0; /* will index into CRC lookup table   */

    printf("\tBefore HI= %02x\n",uchCRCHi);
    printf("\tBefore LO= %02x\n\n",uchCRCLo);
    /* pass through message buffer   */
    //   while (usDataLen--) {
    // uIndex = uchCRCLo ^ *puchMsg++; /* calculate the CRC   */
    // uchCRCLo = uchCRCHi ^ auchCRCHi[uIndex];
    // uchCRCHi = auchCRCLo[uIndex];

    uIndex = uchCRCLo ^ n; /* calculate the CRC   */
    uchCRCLo = uchCRCHi ^ auchCRCHi[uIndex];
    uchCRCHi = auchCRCLo[uIndex];

    printf("\tHI= %02x\n",uchCRCHi);
    printf("\tLO= %02x\n\n",uchCRCLo);

    //    }
    //
    return( uchCRCHi << 8 | uchCRCLo );
}



uint8_t modbus(int n) {
    static int count=0;
    uint8_t status=0xff;
    uint16_t crc=0;

    static uint8_t hiByte=0xff;
    static uint8_t loByte=0xff;

    static uint8_t hiLen=0xff;
    static uint8_t loLen=0xff;

    static uint16_t address=0xffff;
    static uint16_t length=0xffff;

    static uint8_t packetLength=2;

    if( 0 == count && n == RTU ) {
        printf("ME\n");
        status=0;
    }

    if ( 1 == count ) {
        switch( n ) {
            case RHR:
                //                printf("RHR\n");
                status = 0;
                packetLength=8;
                break;
            case WHR:
                //                printf("WHR\n");
                status = 0;
                break;
            default:
                status=0xff;
                break;
        }
    }
    if( 2 == count ) {
        //        printf("Address Hi\n");
        hiByte = (uint8_t)n;
        status = 0;
    }

    if( 3 == count ) {
        //        printf("Address Lo\n");
        loByte = (uint8_t)n;
        status = 0;

        address = (hiByte << 8) | (loByte & 0xff);
    }

    if ( 4 ==  count ) {
        hiLen=(uint8_t) n;
        status = 0;
    }
    if( 5 == count) {
        loLen=(uint8_t) n;
        length = (hiLen << 8) | (loLen & 0xff);

        printf("Read\n");
        printf("Count   = %04x\n",count);
        printf("\tAddress = %04x\n",address);
        printf("\tLength  = %04x\n",length);
        status=0;
    }

    if( count >= packetLength) {
        status=1;
    } else {
        status=0;
    }

    if(status == 0) {
        crc = athCalcCRC( n );
        printf("Count = %02d byte=%02x\n\tCRC     = %04x\n",count,n,crc);
        count++;
    } else if (status == 1){
        count=0;
        status = crc;
    }
    return(status);
}

int main(int argc, char *argv[]) {
    FILE *fd;
    int b;
    uint8_t rc;
    uint16_t crc;

    if(argc != 2) {
        printf("Usage: modbus <filename>\n\n");
        exit(1);
    }

    fd=fopen(argv[1],"r");
    if( NULL == fd) {
        printf("Failed to open file %s\n",argv[1]);
        exit(2);
    }

    while( 0 == feof(fd)) {
        b = fgetc(fd);
        if(b != EOF) {
            rc=modbus( b);
            //            printf("rc=%02x:",rc);
        }
    }
    printf("\n\n");


    return(0);
}
